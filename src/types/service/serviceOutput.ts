...
export type ServiceOutputType = "redirect" | "email" | "contact" | "pds";

export class ServiceOutput {
	type?: ServiceOutputType; // 'redirect', 'pds' is legacy support
	...
	constructor(data: Partial<ServiceOutput>) {
...
export class OutputEmail extends ServiceOutput {
	email: string;
...
export class OutputContact extends ServiceOutput {
	phone?: string;
	email?: string;
	website?: string;
	facebook?: string;
...
export class OutputRedirect extends ServiceOutput {
...
export class OutputPds extends ServiceOutput {
...
export function getOutputPdsFromSource(
	source: ServiceOutput[]
): OutputPds | undefined {
	return getValueFromSource<OutputPds>(source, "pds");
}

export function getOutputContactFromSource(
	source: ServiceOutput[]
): OutputContact | undefined {
	return getValueFromSource<OutputContact>(source, "contact");
}

export function getOutputEmailFromSource(
	source: ServiceOutput[]
): OutputEmail | undefined {
	return getValueFromSource<OutputEmail>(source, "email");
}

function getValueFromSource<T extends ServiceOutput>(
	source: ServiceOutput[] | undefined,
	targetType: ServiceOutputType
): T | undefined {
	if (Array.isArray(source)) {
		return source.find((item): item is T => item.type === targetType);
	}
	return undefined;
}
...
export function prepareOutputPdsForPayload(
	currentValueAtPath: ServiceOutput[],
	newValue: ServiceOutput[]
): ServiceOutput[] {
...
function prepareValueForPayload(
	...
	const newOutput = newValue.find((item) => item.type === targetType);

	// get the relevant output (of targetType) from the currentValueAtPath
	const oldOutput = currentValueAtPath.find((item) => item.type === targetType);
